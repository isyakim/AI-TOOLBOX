...
The Electron main process owns filesystem access, plugin persistence, credential encryption, project indexing, and native dialogs. Renderer input is treated as untrusted and validated at IPC boundaries.
...
- Provider configuration: encrypted with Electron `safeStorage` under `userData`.
